...
export function createJokeContainer(): HTMLElement {
  ...
  container.className =
    "mt-6 sm:mt-8 md:mt-8 w-full max-w-2xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-8 bg-gradient-to-br from-slate-900/40 via-slate-800/30 to-slate-900/40 rounded-2xl backdrop-blur-sm border border-white/10 shadow-xl";

  // Header
  const header = document.createElement("h3");
  header.className =
    "text-lg sm:text-xl md:text-2xl font-bold text-white mb-4 text-center";
  ...
  jokeText.className =
    "text-sm sm:text-base md:text-lg text-gray-100 text-center leading-relaxed";
...
async function loadJoke(): Promise<void> {
  ...
  try {
    ...
    jokeText.textContent = "Loading joke...";
    jokeText.className =
      "text-sm sm:text-base md:text-lg text-gray-400 text-center leading-relaxed italic";
    ...
    jokeText.className =
      "text-sm sm:text-base md:text-lg text-red-300 text-center leading-relaxed";
  }
}
